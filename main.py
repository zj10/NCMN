...
parser.add_argument('--logitDecay', default=1e-3, type=float)
parser.add_argument('--resume', default='', type=str)
parser.add_argument('--randomcrop_pad', default=4, type=float)
parser.add_argument('--ncmn', default=[0, 0], type=float, nargs="+")
...
def main():
    ...
    def create_optimizer(opt):
        ...
        return NDAdam([{'params': scalars.values(), 'weight_decay': opt.weightDecay},
                       {'params': vectors.values(), 'vec_axes': [1, 2, 3]}],
    ...
    def h(sample):
        ...
        y = data_parallel(f, inputs, params, stats, opt.ncmn, sample[2], tuple(range(opt.ngpu)))
    ...
    def on_start_epoch(state):
        ...
        for group in optimizer.param_groups:
            group['lr'] = opt.lr * 0.5 * (1 + math.cos(math.pi * float(epoch) / opt.epochs))
